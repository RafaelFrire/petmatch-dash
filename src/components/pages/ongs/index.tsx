  ...
  <a href={`/ongs/${ong.slug}`} >
    <div className="rounded-[10px] overflow-hidden shadow-md border">
      <div className="relative">
        ...
        <img
          src={"/assets/imageOngs.png"}
          alt="ONG"
          className="w-full h-[294px] object-cover mt-6"
        />
      </div>
      ...
      <div className="flex justify-center p-4">
        <button className="text-primary100 underline font-medium">
          Saiba mais
        </button>
      </div>
    </div>
...
export const OngsListSection:React.FC<OngListProps> = ({ongs}) => {
  return (
    <section className="container mx-auto px-4 py-8  md:w-[75%] ">
      <div className="bg-white rounded-[10px] border-primary100 border-2 border-primary-100 shadow-md p-6 mb-12 md:w-[80%] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4  ">
          {[...Array(6)].map((_, i) => (
            <FilterSelect
              key={i}
              placeholder={i % 2 === 0 ? "Estado" : "Cidade"}
            />
          ))}
        </div>
      </div>
